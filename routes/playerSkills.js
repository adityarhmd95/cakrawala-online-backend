...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (!playerId || !skills) {
      return res.status(400).json({
        message: 'Player ID and skills are required'
      });
    }
    ...
    for (const [skill, {
      ...
      if (!skillFieldMap[skill]) {
        return res.status(400).json({
          message: `Invalid skill type: ${skill}`
        });
      }
      ...
      updateQueries.push(`UPDATE player_skills SET ${levelField} = $${index++}, ${expField} = $${index++} WHERE player_id = $${index++}`);
      values.push(level, exp, playerId);
    }
